...
public class Hashing {
	static char[] letterCheck = "abcdefghijklmnopqrstuvwxyz".toCharArray();
	...
	public static void main(String[] args) throws IOException {
		File testFile = new File("C:\\dictionary.txt"); // this is where the
														// file to be sorted is
														// loaded from
	...
	public static int getHashKey(String word) {
		...
		for (int x = 0; x < word.length(); x++) {
			for (int i = 0; i < letterCheck.length; i++) {
				if (word.charAt(x) == letterCheck[i]) {
					totalValue = totalValue + modPow(i, (int)letterCheck[i], hashTable.length);
					break;
				}
			}
		}
		return totalValue%hashTable.length;
	...
	public static int getDoubleHashKey(String word) {
		...
		int jumpSize = 1;
		int max = hashTable.length/18;
			for(int i = 0; i < word.length(); i++){
				int ran = rand.nextInt();
				if(rand.nextInt() == 0)
					ran++;
				jumpSize = max - (modMult((int)word.charAt(i), ran^2, hashTable.length) % max);
		}
		return jumpSize;
	}
